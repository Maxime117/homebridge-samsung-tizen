...
module.exports = class Input {
    ...
    async setInput() {
        if (this.config.type == 'artMode') {
            await this.device.remote.setArtMode(true);
            return this;
        }
        else if(await this.device.remote.getArtMode()) {
        ...
        } else if (this.config.type == 'apiCall') {
            await this.device.remote.apiCall(this.config.value);
        } else {
